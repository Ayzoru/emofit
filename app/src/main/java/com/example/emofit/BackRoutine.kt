...
class BackRoutine : AppCompatActivity() {

    private lateinit var timerPullUp: TextView
    private lateinit var timerChinUp: TextView
    private lateinit var startPullUp: Button
    private lateinit var startChinUp: Button

    private var pullupTimer: CountDownTimer? = null
    private var chinupTimer: CountDownTimer? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        setContentView(R.layout.activity_back_routine)

        // Initialize TextViews and Buttons
        timerPullUp = findViewById(R.id.timerPullUp)
        timerChinUp = findViewById(R.id.timerChinUp)

        startPullUp = findViewById(R.id.startPullUpButton)
        startChinUp = findViewById(R.id.startChinUpButton)

        // Set the workout duration based on the saved mood
        setWorkoutDurationBasedOnMood()

        // Set click listeners for each start button
        startPullUp.setOnClickListener { startPullUpTimer() }
        startChinUp.setOnClickListener { startChinUpTimer() }
        ...
        pullupTimer = object : CountDownTimer(workoutDuration, 1000) {
            ...
            override fun onFinish() {
                ...
                if (earnedBadge) {
                    ...
                    mediaPlayer?.release()
                    mediaPlayer = MediaPlayer.create(this@BackRoutine, R.raw.workoutcomplete)
                    mediaPlayer?.start()
                }
            }
        }.start()
    ...
    private fun startChinUpTimer() {
        if (isTimerRunning) {
            Toast.makeText(this, "Please finish the current workout timer first.", Toast.LENGTH_SHORT).show()
            return
        }
        isTimerRunning = true
        startChinUp.isEnabled = false

        chinupTimer = object : CountDownTimer(workoutDuration, 1000) {
            override fun onTick(millisUntilFinished: Long) {
                timerChinUp.text = formatTime(millisUntilFinished)
            }

            override fun onFinish() {
                timerChinUp.text = "00:00"
                //Toast.makeText(this@BackRoutine, "Chin Ups Completed! You earned a badge!", Toast.LENGTH_SHORT).show()
                isTimerRunning = false
                startChinUp.isEnabled = true
                chinupTimer = null
    ...
    private fun incrementPoints(points: Int) {
        ...
        if (updatedPoints >= 100) {
            ...
            mediaPlayer = MediaPlayer.create(this@BackRoutine, R.raw.badgeunlock)
            mediaPlayer?.start()
        } else {
